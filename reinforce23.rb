...
#go into results and add views: 0 to each value
all[:response][:results].each do |new_val|
  new_val[:views]= 0

  def read_article(all)
    articles = all[:response][:results].length)-1
    rand_article = rand(0..article)
    all[:response][:results][rand_article][:views]+=1
  end


puts read_article(all)
